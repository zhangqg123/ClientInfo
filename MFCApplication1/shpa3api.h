...
#define	   DE_SIP_REMOTE_PARTY_ID				0x1401
#define	   DE_ROIP_ANI							0x1402
#define	   DE_ROIP_ADPCMPACK					0x1403

#define    DE_LOGON_COMPLETED					0x1410
#define    DE_LOGOFF_COMPLETED					0x1411
...
//-----------------------------------------------------------------------
//call information structure for DST Serials recording card when received 0x70-0x76 D channel events
//-----------------------------------------------------------------------
#define ISDN_CALL_NUMBER_MAXLEN 32
typedef struct tagISDN_CALL_INFO
{
    ULONG               CallRef ;					// call reference, increase
    ULONG               CallSource ;				// call source, incoming or outgoning
    ULONG               Cause ;						// cause
	char CallerNumber[ISDN_CALL_NUMBER_MAXLEN];		// caller number
	char CallerSubAddr[ISDN_CALL_NUMBER_MAXLEN];	// caller sub addr
	char CalledNumber[ISDN_CALL_NUMBER_MAXLEN];		// called number
	char CalledSubAddr[ISDN_CALL_NUMBER_MAXLEN];	// called sub addr
	char ConnectNumber[ISDN_CALL_NUMBER_MAXLEN];	// connect number
	char ConnectSubAddr[ISDN_CALL_NUMBER_MAXLEN];	// connect sub addr
	char RedirectingNumber[ISDN_CALL_NUMBER_MAXLEN];// redirecting number
}ISDN_CALL_INFO, *PISDN_CALL_INFO;
...
enum
{
	...
	E_REG_NOTIFY_RESPONSE                   = 0x008b,   //NOTIFY响应
	E_SIPSTACK_REGSTATUS                    = 0x008c,   // Add by jince for OS-9530 增加协议栈注册事件
	E_HOLDING_CALL                          = 0x008d,   //holdcall event	
	E_SIP_RESPONSE                          = 0x008e,   //Add by jince for OS-10659 Add 181 event output 2017.8.21
	E_RCV_IPR_IGMP_MESSAGE					= 0x008f,	//IPR(ROIP)receive IGMP message, generate a event if type is 0x11(query)
	E_SYS_Lost_Interrupt					= 0x0090,   //add by xiazeyang for DS-38666
	E_SIPSTACK_PCMSTATE = 0x0091,	//add by laixuebin OS-12080
	E_SIP_SUBSCRIBE                         = 0x0092,   //add by jince for OS-13231 上抛SUBSCRIBE  2018.12.3
	E_RecEnergy                             = 0x0093,   //录音能量
    MAX_EVENT_SIZE
};
...
#define MAX_SIPIP_BINDCHNUM 4096
...
//++++start++++added by xzw for sipserver
typedef struct tag_RegResp
{
    int  nAuth;//鉴权参数，取值范围为0和1
	char szPassword[MAX_SIPIP_ADDR_LENGTH];//用户密码
	char szUsername[MAX_SIPIP_ADDR_LENGTH];
}RegResp;

typedef struct tag_Reg_Info
{
    int   nRegIndex;//注册用户索引号
	int   nRegState;//注册状态：0表示注册失败；1表示注册成功
	int   nT0State;//用于定时器T0状态控制
	char  szTelNo[MAX_SIPIP_ADDR_LENGTH];//注册用户名
	char  szClientAddress[MAX_SIPIP_ADDR_LENGTH];//注册客户端IP地址
	char  szClientPort[MAX_SIPIP_ADDR_LENGTH];//注册终端端口号
	char  szUserID[MAX_SIPIP_ADDR_LENGTH];//认证用户名
	int   nExpires;
	int   nRegCountForAuthT0;//用于定时器T0计数
	int   nRegCountForAuthT1;//用于定时器T1计数
...
//++++end++++++added by xzw for sipserver
//-----------------------------------------------------------------------
...
typedef struct tagCOMPUTER_INFO
{
	...
	char szCPUInfo[1000];					// CPU information
...
#define IP_IGMP 2  // IGMP Protocol
#define IP_TCP	6  // TCP Protocol
#define IP_UDP	17 // UDP Protocol

typedef struct tagIGMP_QUERY_INFO
{
	UCHAR ucType;
	UCHAR ucMaxRespTime;
	char szSrcIPAddr[30];
	char szMulticastIPAddr[30];
}IGMP_QUERY_INFO,*PIGMP_QUERY_INFO;
...
enum
{
	...
	PTL_NORTEL_UNISTIM	= 14,	// Nortel Unistim Protocol
	PTL_MAX
};

#define IPR_MAX_PROXY_LIST 20
...
typedef struct
{
	...
	BOOL bGrouupCallCompatible;
	USHORT ProxyIPAddressAdditionalCount;
	ULONG ProxyIPAddressAdditional[IPR_MAX_PROXY_LIST];
} IPR_SIP_CFGS;
...
typedef struct  
{
	IPR_MONITOR_ITEM NortelUNIStim;	// Nortel UNIStim Parameters
...
typedef struct
{
    union
    {
		...
		IPR_NORTEL_UNISTIM_CFGS Nortel_UNIStim;	// Nortel UNIStim parameters
    };

}IPR_MONITOR_CFGS, *PIPR_MONITOR_CFGS;

#define IPR_MAX_CALL_ID_SIZE 100
#define MAX_LCD_SIZE 512
...
typedef struct tagStationInfoEx
{
	...
	char szLCDDisplayInfo[MAX_LCD_SIZE];	// LCD display infomation
}StationInfoEx,*PStationInfoEx;

typedef struct tagROIPANIInfo
{
	UCHAR CallType;
	UCHAR CallFormat;
	UCHAR szReserved1[9];
	UCHAR szUnitID[34];
	UCHAR szStatusID[10];
	UCHAR szCallingID[10];
	UCHAR szCallingGroupID[10];
	UCHAR szReserved2[21];
}ROIPANIInfo,*PROIPANIInfo;

typedef struct tagROIPADPCMPackInfo
{
	UCHAR Frequency;
	UCHAR DTMF;
	UCHAR MiscellaneousBits;
}ROIPADPCMPackInfo,*PROIPADPCMPackInfo;

typedef struct registering_message registering_message_t;//added by xzw for sipserver
...
typedef BOOL (WINAPI *RECORDMEMBLOCKHANDLERA)(int ch, int nEndReason, LPBYTE pucBuf, LPBYTE pucBufInbound, LPBYTE pucBufOutbound, DWORD dwStopOffset, PVOID pV);
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	int WINAPI SsmSetLogAttributeA(int nLogPeriodMode, int nLogCreatePeriod,int nLogMaxKeep,int nLogMaxPeriod,LPCSTR pLogFilePath);
	...
	int WINAPI SsmGetLogAttributeA(PINT pLogPeriodMode,PINT pLogCreatePeriod,PINT pLogMaxKeep,PINT pLogMaxPeriod,LPSTR pLogFilePath);		
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	int WINAPI SsmPlayFileW(int ch, LPCWSTR  pszFileName, int nFormat, DWORD dwStartPos, DWORD dwLen);//added by gy for  OS-8353 2016.7.5
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	int WINAPI SsmGetRecSamplerate(int ch);
	int WINAPI SsmRecToFile(int ch, LPCSTR pszFileName, int nFormat, DWORD dwStartPos, DWORD dwBytes, DWORD dwTime, int nMask);
int WINAPI SsmRecToFileW(int ch, LPCWSTR  pszFileName, int nFormat,
                        DWORD dwStartPos, DWORD dwBytes, DWORD dwTime, int nMask); //added by gaoyong for OS-8353 2016.6.14
	...
	int WINAPI SsmRecToFileEx(int ch, LPCSTR pszFileName, int nFormat,
		...
		int WINAPI SsmRecToFileAW(int ch, LPCWSTR pszFileName, int nFormat,
							 DWORD dwStartPos, DWORD dwBytes, DWORD dwTime, int nMask,
							 LPRECTOMEM pfnRecToMem);
	int WINAPI SsmRecToFileBW(int ch, LPCWSTR pszFileName, int nFormat,
							 DWORD dwStartPos, DWORD dwBytes, DWORD dwTime, int nMask,
							 LPRECTOMEMB pfnRecToMem, PVOID pV);
	int WINAPI SsmRecToFileExW(int ch, LPCWSTR pszFileName, int nFormat,
							  DWORD dwStartPos, DWORD dwBytes, DWORD dwTime, int nMask,
							  BOOL bSaveToFileOnBargin, DWORD dwRollbackTime);

	int WINAPI SsmPauseRecToFile(int ch);
	int WINAPI SsmPauseRecToFileEx(int ch, BOOL bInsertSilence);
	...
	int WINAPI SsmRecordMemBlockA(int ch, LPCSTR pszFileName, int nFormat, LPBYTE pBuf, LPBYTE pBufInbound, LPBYTE pBufOutbound,
							   	DWORD dwBufSize, RECORDMEMBLOCKHANDLERA OnRecMemBlockDoneA, PVOID pV);
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	int WINAPI SsmGetCallNum(int ch, LPSTR szCallerNum, LPSTR szCalleeNum);
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	int WINAPI SsmAutoDialAgent(int ch, LPCSTR szCallerPhoNum,LPCSTR szCalleePhoNum);//added by xzw for sipserver
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    int  WINAPI SpyGetCallerName(int nCic, char *pcCallName);//added by xjw for OS-6997 2015.8.23
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	int WINAPI SsmSendIsdnMsuEx(int nPcmId,int nMsgType, int nMsgLen, PUCHAR pucMsuBuf);//added by xiazeyang for DS-47725,2016.7.7
	int WINAPI SsmGetIsdnMsuEx(int nPcmId,int nMsgType, PUCHAR pucMsuBuf, PWORD wMsgLen);////added by xiazeyang for DS-47725,2016.7.13
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	int WINAPI SsmSipChEnableSrtp(int nChFrom, int nChTo, BOOL bSrtp);
	...
	int WINAPI SsmSipUnBindChWithOutRegInfo(int nChID);
    ...
    int WINAPI SsmSipRegResponse(LPCSTR pszUserName, RegResp *pstRegResp);//added by xzw for sipserver
	int WINAPI SsmSipGetUserInfoByIndex(int nRegIndex,Reg_Info *pstReg_Info);//added by xzw for sipserver
	int WINAPI SsmSipRemoveRegInfo(LPCSTR pszUserName);//DS-39675
	int WINAPI SsmSipStackAddWhiteOrBlackList(char *pszSipMsgIp,int nWhiteOrBlack);
	int WINAPI SsmSipStackRemoveWhiteOrBlackList(char *pszSipMsgIp,int nWhiteOrBlack);
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	int WINAPI SsmIPRAddSessionToMap(int nBId, int nStationId, LPSTR szAddrPrimary, int nPortPrimary, LPSTR szAddrSecondary, int nPortSecondary);
	int WINAPI SsmSetNotchfilter(int ch, BOOL bNotchfilter, DWORD wFilterHz);

	int WINAPI SsmGetMaxFlashFilterTime(int ch);
	int WINAPI SsmSetMaxFlashFilterTime(int ch, WORD wTime);
#else
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//@@@@@@@@@@@@@@@@@@@@@@@@@@@ Gateway Client API @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
